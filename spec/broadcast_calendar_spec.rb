...
describe BroadcastCalendar do
  ...
  describe '::VERSION' do
    it { expect(described_class::VERSION).to eql('1.1.3') }
  end
    ...
    it 'converts months and years to ranges' do
      dates.each do |year, months|
        months.each do |month, range|
          expect(described_class.dates_for(month, year)).to eql range
        end
      end
    end
  end
  ...
  describe '#dates_for_week' do
    it { expect(described_class.dates_for_week(2017, 1)).to eql Date.new(2016, 12, 26)..Date.new(2017, 1, 1) }
    it { expect(described_class.dates_for_week(2017, 4)).to eql Date.new(2017, 1, 16)..Date.new(2017, 1, 22) }
    it { expect(described_class.dates_for_week(2018, 22)).to eql Date.new(2018, 5, 28)..Date.new(2018, 6, 3) }
    it { expect(described_class.dates_for_week(2018, 52)).to eql Date.new(2018, 12, 24)..Date.new(2018, 12, 30) }
  end
  ...
  describe '#month_and_year_for' do
    it 'returns the broadcast month and year for a given year' do
      dates.each do |year, months|
        months.each do |month, range|
          range.each do |date|
            expect(described_class.month_and_year_for(date)).to eql [month, year]
          end
        end
      end
    end
  ...
  describe '#weeks_for' do
    context 'when it is July 2010' do
      let(:weeks) { described_class.weeks_for(7, 2011) }

      it { expect(weeks[0]).to eql Date.new(2011, 6, 27)..Date.new(2011, 7, 3) }
      it { expect(weeks[1]).to eql Date.new(2011, 7, 4)..Date.new(2011, 7, 10) }
      it { expect(weeks[2]).to eql Date.new(2011, 7, 11)..Date.new(2011, 7, 17) }
      it { expect(weeks[3]).to eql Date.new(2011, 7, 18)..Date.new(2011, 7, 24) }
      it { expect(weeks[4]).to eql Date.new(2011, 7, 25)..Date.new(2011, 7, 31) }
    end

    context 'when it is August 2010' do
      let(:weeks) { described_class.weeks_for(8, 2010) }

      it { expect(weeks[0]).to eql Date.new(2010, 7, 26)..Date.new(2010, 8, 1) }
      it { expect(weeks[1]).to eql Date.new(2010, 8, 2)..Date.new(2010, 8, 8) }
      it { expect(weeks[2]).to eql Date.new(2010, 8, 9)..Date.new(2010, 8, 15) }
      it { expect(weeks[3]).to eql Date.new(2010, 8, 16)..Date.new(2010, 8, 22) }
      it { expect(weeks[4]).to eql Date.new(2010, 8, 23)..Date.new(2010, 8, 29) }
    end
  end
end
